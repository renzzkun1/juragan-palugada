...
// State internal untuk melacak kontak WA aktif yang sedang dipilih oleh pemain
let selectedNpcId = null; 

/* STREAMING_CHUNK: Menginisialisasi delegasi klik event listener */
/**
...
export function initUI() {
    ...
    gameRoot.addEventListener('click', (e) => {
        ...
        // Tombol Memilih Kontak WhatsApp
        const contactBtn = e.target.closest('[data-action="select-contact"]');
        if (contactBtn) {
            selectedNpcId = contactBtn.getAttribute('data-id');
            renderScreen('chat');
            return;
        }

        // Tombol Kembali ke Daftar Kontak (WhatsApp Back Button)
        const backChatBtn = e.target.closest('[data-action="back-to-contacts"]');
        if (backChatBtn) {
            selectedNpcId = null;
            renderScreen('chat');
            return;
        ...
        if (removeApiKeyBtn) {
            // Menggunakan UI Toast dan Konfirmasi custom agar tidak memakai dialog browser kaku
            if (window.confirm("Apakah Anda yakin ingin menghapus API Key dari game ini?")) {
                removeApiKey();
                selectedNpcId = null;
                showToast("API Key berhasil dicabut!", "info");
                renderScreen('chat');
            }
            return;
        }
        ...
        if (buyUpgradeBtn) {
            ...
            if (window.confirm("Apakah Anda yakin ingin berinvestasi di upgrade ini?")) {
        ...
        if (resetBtn) {
            if (window.confirm("Apakah Anda yakin ingin mengulang permainan dari hari ke-1? Semua progres akan hilang.")) {
                resetGame();
...
/* STREAMING_CHUNK: Mengelola pengiriman pesan AI berdasarkan kontak aktif */
/**
 * Memproses pengiriman pesan dari pemain ke asisten AI (Multi-NPC)
 */
async function handleSendChat() {
    const input = document.getElementById('chat-input');
    if (!input || !input.value.trim() || !selectedNpcId) return;

    const userMessage = input.value.trim();
    input.value = ''; // Kosongkan kolom input dengan cepat demi fluiditas UI

    // Ambil riwayat chat spesifik untuk NPC yang sedang dipilih
    const history = npcChatHistories[selectedNpcId];
    if (!history) return;

    // 1. Masukkan pesan pemain ke riwayat obrolan NPC tersebut
    history.push({ role: 'user', content: userMessage });
    renderScreen('chat');
    scrollToBottom();

    // 2. Tampilkan status mengetik "..." untuk NPC aktif
    history.push({ role: 'assistant', content: '', isTyping: true });
    renderScreen('chat');
    scrollToBottom();

    try {
        // 3. Kirim pesan ke modul Groq Service dengan menyertakan ID NPC tujuan
        const reply = await sendNpcChatMessage(selectedNpcId, userMessage);

        // Hapus status mengetik, lalu ganti dengan jawaban dari AI
        const typingIndex = history.findIndex(msg => msg.isTyping);
        if (typingIndex !== -1) {
            history.splice(typingIndex, 1);
        }
        history.push({ role: 'assistant', content: reply });
    } catch (error) {
        // Hapus status mengetik dan berikan pesan kesalahan fiktif agar imersi terjaga
        const typingIndex = history.findIndex(msg => msg.isTyping);
        if (typingIndex !== -1) {
            history.splice(typingIndex, 1);
        }
        history.push({ 
            role: 'assistant', 
            content: `Aduh Bos... Jaringan lagi lemot banget nih! Satelit grosiran kayaknya lagi keganggu cuaca buruk di Glodok. Coba ketik lagi deh ya!` 
        });
...
/* STREAMING_CHUNK: Mengatur pergantian hari dan layar game over */
/**
...
function handleTransaction(itemId, type) {
    const qtyInput = window.prompt(`Masukkan jumlah unit yang ingin di-${type === 'buy' ? 'beli' : 'jual'}:`, "1");
...
/* STREAMING_CHUNK: Menyusun renderScreen dan kerangka navigasi bawah */
/**
...
export function renderScreen(screenId) {
    ...
    if (screenId === 'chat' && selectedNpcId) {
...
/* STREAMING_CHUNK: Merender tampilan dashboard dan pasar */
/**
...
/* STREAMING_CHUNK: Merender tampilan gudang, shop, dan loan */
/**
    ...
    return `
        <div class="pt-2 space-y-4 animate-page-enter">
            <div class="bg-gradient-to-br from-purple-900/40 to-slate-900 border border-purple-500/20 rounded-2xl p-4 text-center">
                ...
                <p class="text-xs text-slate-400 leading-relaxed max-w-xs mx-auto text-center">Tingkatkan aset, beli asuransi, dan perluas jangkauan Anda di pasar untuk memaksimalkan cuan harian.</p>
            </div>
    ...
    return `
        <div class="pt-2 space-y-4 animate-page-enter">
            <div class="bg-gradient-to-br from-rose-950/40 to-slate-900 border border-rose-500/20 rounded-2xl p-4 text-center">
                ...
                <p class="text-xs text-slate-400 leading-relaxed max-w-xs mx-auto mb-3 text-center">"Utangmu terus membengkak sebesar 1% setiap malamnya akibat bunga. Bayar sebelum waktu habis atau tokomu kusegel!"</p>
            ...
            ` : `
                <div class="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-6 text-center">
                    ...
                    <p class="text-xs text-slate-300 leading-relaxed text-center">Selamat! Anda berhasil melepaskan diri dari kejaran rentenir. Sekarang, fokuslah mengumpulkan kekayaan sebanyak-banyaknya hingga hari terakhir!</p>
                </div>
            `}
        </div>
    `;
...
/* STREAMING_CHUNK: Merender tampilan Chat AI WhatsApp-style Hub */
/**
 * Tampilan Halaman Obrolan AI (WA Multi-Persona dengan Tampilan Hub WhatsApp)
 */
function renderChat() {
        ...
        return `
            <div class="pt-2 space-y-4 animate-page-enter">
                <div class="bg-gradient-to-br from-amber-500/10 to-slate-900 border border-amber-500/20 rounded-2xl p-5 text-center">
                    ...
                    <p class="text-xs text-slate-400 leading-relaxed max-w-xs mx-auto mb-4 text-center">
                        Untuk mengaktifkan asisten AI WhatsApp multi-kontak yang bisa membaca sisa uang, isi gudang, dan fluktuasi pasar secara real-time, masukkan API Key Groq Cloud Anda.
                    </p>
                    ...
                    <div class="space-y-3 text-left">
                        <label class="text-[10px] text-slate-500 font-bold uppercase tracking-wider block font-Outfit">Masukkan Groq API Key:</label>
    ...
    // Skenario Kontak List (WhatsApp Hub): Belum ada kontak yang dipilih
    if (!selectedNpcId) {
        const contactIds = Object.keys(NPC_PROFILES);
            ...
            <div class="pt-2 flex flex-col h-[calc(100vh-230px)] animate-page-enter">
                <div class="flex justify-between items-center mb-4 border-b border-slate-800/60 pb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <h4 class="font-bold text-slate-100 text-sm">Juragan Messenger (WA AI)</h4>
                            <span class="text-[9px] text-emerald-400 font-extrabold uppercase tracking-widest flex items-center gap-1">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> 3 Kontak Terhubung
                            </span>
                        </div>
                    </div>
                    <button data-action="remove-api-key" class="text-[10px] text-red-400/80 hover:text-red-400 font-bold underline transition">
                        Cabut API Key
                    </button>
                </div>

                <div class="space-y-3 flex-1 overflow-y-auto scrollbar-none">
                    ${contactIds.map(npcId => {
                        const profile = NPC_PROFILES[npcId];
                        const history = npcChatHistories[npcId] || [];
                        const lastMessage = history.length > 0 ? history[history.length - 1].content : profile.salamMulai;
                        
                        // Batasi panjang teks pesan terakhir agar rapi di daftar obrolan
                        const slicedMessage = lastMessage.length > 40 ? lastMessage.slice(0, 37) + '...' : lastMessage;

                        return `
                            <div data-id="${npcId}" data-action="select-contact" class="flex gap-3.5 items-center bg-slate-900/40 border border-slate-800/80 p-4 rounded-2xl cursor-pointer hover:bg-slate-800/40 hover:border-slate-700/80 transition duration-150 active:scale-98">
                                <div class="w-11 h-11 rounded-full bg-slate-800 border border-slate-700/80 flex items-center justify-center text-2xl shadow-inner flex-shrink-0">
                                    ${profile.avatar}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-baseline mb-0.5">
                                        <h4 class="font-bold text-slate-100 text-xs sm:text-sm truncate">${profile.nama}</h4>
                                        <span class="text-[9px] text-slate-500 font-semibold">Online</span>
                                    </div>
                                    <p class="text-[11px] text-slate-400 truncate leading-relaxed">${slicedMessage}</p>
                                </div>
                                <span class="text-xs text-amber-500 font-extrabold">›</span>
                            </div>
                        `;
                    }).join('')}
                </div>
    ...
    // Skenario Jendela Chat Aktif: Sedang mengobrol dengan salah satu NPC
    const activeProfile = NPC_PROFILES[selectedNpcId];
    const activeHistory = npcChatHistories[selectedNpcId] || [];

    return `
        <div class="pt-2 flex flex-col h-[calc(100vh-230px)] animate-page-enter">
            <!-- Header Chat WhatsApp -->
            <div class="flex justify-between items-center mb-3 border-b border-slate-800/60 pb-3">
                <div class="flex items-center gap-2.5">
                    <button data-action="back-to-contacts" class="w-8 h-8 rounded-full bg-slate-800 hover:bg-slate-700/80 border border-slate-700/50 flex items-center justify-center text-slate-300 font-bold transition mr-1">
                        ←
                    </button>
                    <div class="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-xl flex-shrink-0">
                        ${activeProfile.avatar}
                    </div>
                    <div>
                        <h4 class="font-bold text-slate-100 text-xs sm:text-sm">${activeProfile.nama}</h4>
                        <span class="text-[9px] text-emerald-400 font-bold uppercase tracking-wider flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> ${activeProfile.status}
                        </span>
                    </div>
                </div>
            ...
            <div id="chat-window" class="flex-1 bg-black/40 border border-slate-800/60 rounded-2xl p-3.5 overflow-y-auto space-y-3.5 mb-3 scrollbar-none">
                ${activeHistory.map(msg => {
                    ...
                    const avatar = isUser ? '👤' : activeProfile.avatar;
                    const name = isUser ? 'Anda' : activeProfile.nama;
                        ...
                        return `
                            <div class="flex gap-2.5 items-start">
                                ...
                                <div class="flex-1 space-y-0.5">
                                    <span class="text-[10px] text-slate-500 font-bold uppercase">${activeProfile.nama}</span>
                    ...
                    return `
                        <div class="flex gap-2.5 items-start ${alignment}">
                            ...
                            <div class="flex-1 space-y-0.5 max-w-[80%] ${isUser ? 'text-right' : 'text-left'}">
                                <span class="text-[10px] text-slate-500 font-bold uppercase block">${isUser ? 'Anda' : name}</span>
            ...
            <!-- Input Bar Chat -->
            <div class="flex gap-2">
                <input type="text" id="chat-input" placeholder="Ketik pesan untuk ${activeProfile.nama}..." class="flex-1 bg-slate-900/60 border border-slate-800 focus:border-amber-500/40 rounded-xl px-4 py-3 text-xs text-slate-200 outline-none transition duration-150">
...
/* STREAMING_CHUNK: Merender toast notification pop-up */
/**
